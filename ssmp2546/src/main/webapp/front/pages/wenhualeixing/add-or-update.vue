<template>
	<view class="content">
		<form class="app-update-pv">
				...
				<view :style='{"width":"170rpx","fontSize":"28rpx","color":"rgba(255, 153, 0, 1)","textAlign":"left"}' class="title">文化类型</view>
				<input :style='{"boxShadow":"0 0 0px rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(255, 153, 0, 1)","borderRadius":"16rpx","color":"rgba(0, 0, 0, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' :disabled="ro.wenhualeixing" v-model="ruleForm.wenhualeixing" placeholder="文化类型"></input>
			</view>
	...
	export default {
		...
		async onLoad(options) {
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`wenhualeixing`, this.ruleForm.id);
		...
		methods: {
			...
			async onSubmitTap() {
																																																																if((!this.ruleForm.wenhualeixing)){
					this.$utils.msg(`文化类型不能为空`);
					return
				}
																																								if(this.ruleForm.id){
					await this.$api.update(`wenhualeixing`, this.ruleForm);
				}else{
					await this.$api.add(`wenhualeixing`, this.ruleForm);
				}
